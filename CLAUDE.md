...
- `Users.razor` (`/manage/users`, `[Authorize(CanAdminHome)]`) — member management, **scoped**: a `HomeAdmin` sees/manages only their home's members (create member, change role up to `HomeAdmin`, delete) and cannot touch a global `Admin`; an `Admin` sees all users with each one's home and can pick any home/role. Last-admin guards prevent demoting/deleting the final `Admin`.
- `Register.razor` (`/register`, `[AllowAnonymous]`) — public self-registration: creates the user, assigns `HomeAdmin`, creates a **new** home, writes the membership, mints the token, auto-signs-in. Rolls the account back if the home name is taken.
- `Login.razor` / `Denied.razor` use `EmptyLayout`.
...
- **Roles** (`Core/Models/Roles.cs`): hierarchical `Read ⊂ Manage ⊂ HomeAdmin ⊂ Admin`, one per user. `Read`/`Manage`/`HomeAdmin` act within the user's own home; **`Admin` is the global super-admin** (all homes/users). Policies in `AuthorizationPolicies` + `Program.cs`: `CanRead`, `CanManage`, **`CanAdminHome`** (`HomeAdmin`+`Admin` — own-home member management), `CanAdmin` (`Admin` only — global). Own-home scoping is enforced in the pages/services, not by the policy.
